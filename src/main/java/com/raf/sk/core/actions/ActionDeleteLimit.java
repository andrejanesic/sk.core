...
/**
 * Korisnička radnja oduzimanja ograničenja za zadati čvor {@link com.raf.sk.core.repository.INode} (ili za celo skladište ako je
...
public class ActionDeleteLimit implements IAction {
    ...
    public Object undo() {
        ...
        try {
            ...
            //noinspection ConstantConditions
            if (!(Core.getInstance().UserManager().getUser().hasPrivilege(target.getPath(), PrivilegeType.LIMIT_ADD) ||
                    Core.getInstance().UserManager().getUser().hasPrivilege(target.getPath(), PrivilegeType.LIMIT_ALL) ||
